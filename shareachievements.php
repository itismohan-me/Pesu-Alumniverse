<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        $result = mysqli_query($con, "SELECT MAX(id) AS max_id FROM achievements");
        $row = mysqli_fetch_assoc($result);
        $next_id = $row['max_id'] + 1;
        if(move_uploaded_file($file_loc,$folder.$final_file)){
            $sql="INSERT INTO achievements (id,name,email,achievement,description,file_name) values ($next_id,'$name','$alumniEmail','$achievement','$description','$final_file')";
